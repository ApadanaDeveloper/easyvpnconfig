...
display_header() {
	...
	# Default values
	DEFAULT_VIRTUAL_IP="10.144.144.1"
	DEFAULT_DEVICE_TYPE="tap"
	DEFAULT_LISTEN_PORT="3210"
	DEFAULT_PASSWORD="1qaz2wsx66"
	DEFAULT_ENCRYPTION="yes"
	
	# Virtual IP address
	echo -ne "${RED}[+]${NC} Virtual IP Address (Default ${DEFAULT_VIRTUAL_IP}): "
	read -r VIRTUAL_IP
	VIRTUAL_IP="${VIRTUAL_IP:-$DEFAULT_VIRTUAL_IP}"
	
	# Device Type
	echo
	echo -ne "${GREEN}[-]${NC} Device type (${DEFAULT_DEVICE_TYPE}): "
	read -r DEVICE_TYPE
	DEVICE_TYPE="${DEVICE_TYPE:-$DEFAULT_DEVICE_TYPE}"
	
	if [[ "$DEVICE_TYPE" != "tun" && "$DEVICE_TYPE" != "tap" ]]; then
		colorize red "Invalid device type, changing to default (${DEFAULT_DEVICE_TYPE})" bold
		DEVICE_TYPE="$DEFAULT_DEVICE_TYPE"
	fi
	
	# Listen Port
	echo
	echo -ne "${GREEN}[-]${NC} Listen Port (Default ${DEFAULT_LISTEN_PORT}): "
	read -r LISTEN_PORT
	LISTEN_PORT="${LISTEN_PORT:-$DEFAULT_LISTEN_PORT}"
	
	# Password
	echo
	echo -ne "${GREEN}[-]${NC} Password (Default password is used if left blank): "
	read -r PASSWORD
	PASSWORD="${PASSWORD:-$DEFAULT_PASSWORD}"
	
	# Encryption
	echo
	echo -ne "${GREEN}[-]${NC} Encryption Enabled (${DEFAULT_ENCRYPTION}): "
	read -r ENCRYPTION
